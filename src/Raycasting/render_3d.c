...
/*   Updated: 2024/10/06 12:22:39 by omghazi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void draw_wall(int x, t_cub3D *cub)
{
    ...
    else if (cub->rays[x].wall_content == 3)
        current_texture = cub->door_img;  // Door texture
    for (int y = from_y; y < to_y; y++)
    {
        int texture_y = (int)texture_pos % tex_height;  // Ensure texture_y wraps
        texture_pos += step;
        uint32_t color = get_texture_pixel(current_texture, texture_x, texture_y);
        // color |= 0xFF000000;
